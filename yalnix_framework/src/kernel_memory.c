...
/*
 * Kernel memory management for Region 0 page tables, kernel stack handling,
 * and kernel heap growth.
 */
...
/* Enable hardware virtual memory support. */
void enable_vm() {
  WriteRegister(REG_VM_ENABLE, 1);
  vm_enabled = true;
}

/* Initialize the kernel heap break pointer. */
void init_kernel_brk() {
  current_brk_page = _orig_kernel_brk_page;
}

/* Set up the kernel Region 0 page table and reserve kernel pages. */
...
/* Adjust kernel brk and allocate or free kernel heap pages. */
...
/* Free a range of kernel pages allocated via SetKernelBrk. */
static void undo_allocation(int first_vpn, int last_vpn) {
  for (int vpn = first_vpn; vpn < last_vpn; vpn++)
    dealloc_page(vpn);
}

/* Copy the kernel stack and context into a new process control block. */
...
/* Switch kernel stack mappings during a process swap. */
...
/* Allocate a kernel region 0 page and map it into the kernel page table. */
static bool alloc_page(int vpn, int prot) {
  int pfn = find_frame();
  if (pfn == -1)
    return false;
...
/* Deallocate a region 0 page and free its physical frame. */
static bool dealloc_page(int vpn) {
  if (vpn < REGION0_VPNS && vpn >= 0) {
...
/* Unmap a region 0 page without releasing the frame. */
static int unmap_no_free(int vpn) {
  if (vpn < REGION0_VPNS && vpn >= 0) {
    int pfn = destroy_pte(region0_pt, vpn);
    WriteRegister(REG_TLB_FLUSH, vpn << PAGESHIFT);
